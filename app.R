...
server <- function(input, output) {
  output$df_res <-renderTable({
    ...
    df <- filter(df, Recip_State== input$state_name)
    
    wa_df <- select(df, Recip_County, Series_Complete_Yes, WA_ABOVE_AVERAGE)
    ...
    aa_df <- select(df, Recip_County, Series_Complete_Yes, AA_ABOVE_AVERAGE)
    ...
    ba_df <- select(df, Recip_County, Series_Complete_Yes, BA_ABOVE_AVERAGE)
    ...
    ia_df <- select(df, Recip_County, Series_Complete_Yes, IA_ABOVE_AVERAGE)
    ...
    na_df <- select(df, Recip_County, Series_Complete_Yes, NA_ABOVE_AVERAGE)
    ...
    h_df <- select(df, Recip_County, Series_Complete_Yes, H_ABOVE_AVERAGE)
    ...
    dfs <- melt(dfs, id.vars = 'ABOVE_AVG', variable.name = 'complete')
    return(dfs)
  })
}
